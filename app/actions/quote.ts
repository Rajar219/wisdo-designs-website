...
export async function submitQuoteForm(data: QuoteFormData) {
  ...
  if (validatedFields.data.botcheck) {
    console.warn("Spam detected via honeypot field in quote. Suppressing submission.");
  ...
  try {
    ...
    const emailBody = `
New WISDO TECH Quote Request (Lead Record)
...
Date/Time: ${date}
    ...
    if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS || !CONTACT_EMAIL) {
      ...
      console.log("--- SIMULATED QUOTE EMAIL CONTENT ---");
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `New Quote Request: ${serviceRequired} - ${businessName || name}`,
      text: emailBody,
    });
